...
<div id="divMain" style="border-radius:5px;">
	<div id="divSearch" style="border-radius:5px;">
		<form id="form2" name="form2" method="post" action="searchAllQuestion">
		    科目:
        	    <select style="margin-right:10px;width:90px;height:25px;" name="course">
        		<option value="0">所有</option>
        		<option value="1">数据结构</option>
        		<option value="2">操作系统</option>
        		<option value="3">计算机网络</option>
        		<option value="4">计算机组成原理</option>
        		</select>
			题型:
			<select  style="margin-right:10px;width:90px;height:25px;" name="type">
				<option value="0">所有</option>
				<option value="1">选择题</option>
				<option value="2">非选择题</option>
			</select>
			难度:
			<select style="margin-right:10px;width:90px;height:25px;" name="difficuty">
				<option value="0">所有</option>
			...
			内容:
			<input type="text" style="width:80px;height:18px;" name="name">
			<input type="submit" style="margin-left:20px;width:50px;height:25px;" name="搜索" value="搜索" >
		</form>
	</div>
	...
	<div id="divQuestion" style="border-radius:5px;">
		<table id="tableQuestion" style="border-radius:5px;">
			...
			<%
			    ...
			    for(Question temp : questions)
			    {
			        ...
			        {
			            out.println("<tr><td>"
			                                 +i+".难度:"+temp.getQuestionDifficulty()+" " +
			        ...
			        {
			            out.println("<tr><td>"+i+".难度:"+temp.getQuestionDifficulty()+" " +
                        					temp.getQuestionContent()+
                        					"("+"分值 "+temp.getQuestionScore()+")"+
                        					"</td></tr>");
		...
		<table id="tableButtons" style="border-radius:5px;">
			<tr>
				<td id="tdBack"><a href="Login.jsp">退出</a></td>
			</tr>
		</table><%-- tableButtons --%>
	</form>
</div><%-- divMain --%>
